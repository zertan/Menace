...
for fn in "${files[@]}"; do
	echo "Aligning $fn:"
	>&2 echo "$fn:"
	bowtie2 -k 8 --very-sensitive-local --score-min L,0,1.2 -p $CPUCORES -x $REFPATH/Index/$REFNAME -1 "$fn"_1"$fileEnding" -2 "$fn"_2"$fileEnding" -S "$fn".sam --un-gz single_un_"$fn".fastq.gz --un-conc-gz paired_un_"$fn".fastq.gz --no-unal
done
